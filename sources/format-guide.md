...
**Важно:** Pandoc при конвертации из Word может генерировать grid-таблицы (`+---+---+`), которые не рендерятся большинством Markdown-движков. Скрипт `convert.py` автоматически конвертирует их в pipe-формат. Для ручного исправления:
...
### 5. Таблицы
- Используется только pipe-формат (`| --- |`), НЕ grid-формат Pandoc (`+---+`)
- `convert.py` автоматически конвертирует grid → pipe при конвертации из Word
- Для исправления существующих файлов: `python3 fix_grid_tables.py <путь>`
...
- [ ] Таблицы в pipe-формате (`| --- |`), нет grid-таблиц Pandoc (`+---+`)
