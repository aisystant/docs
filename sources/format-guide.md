...
В файле `index.md` каждого раздела курса **обязательно** добавляется поле `aisystant_code` в YAML Frontmatter:
...
title: "Название раздела"
order: 1
aisystant_code: "ABC123"
---
```

**Правила:**
- Поле `aisystant_code` указывается **только в index.md** (корневой файл раздела)
- Код **обязательно запросить у автора** исходного материала перед публикацией
- Без указания `aisystant_code` раздел не будет корректно привязан к курсу в Aisystant
- Код уникален для каждого раздела и выдается администратором платформы

**Пример index.md с aisystant_code:**
...
order: 2
aisystant_code: "SM2024-01-02"
---


# Физический мир

Введение в раздел о физическом мире...
```
