...
### Чек-лист проверки результата конвертации

После конвертации рекомендуется пройти по полному чек-листу. Валидатор (`validate.py`) покрывает все проверки автоматически.

| # | Проверка | Что именно проверяем | Автоматически |
|---|----------|----------------------|:-------------:|
| 1 | **YAML front matter** | Каждый .md имеет блок `---` с полями `title`, `order`; у подразделов есть `type`; у корневого `index.md` есть `aisystant_code` | да |
| 2 | **Заголовок H1** | Ровно один `# Заголовок` после front matter; совпадает с `title` в YAML | да |
| 3 | **Две пустые строки** | После закрывающего `---` перед `# H1` ровно две пустые строки | да |
| 4 | **Типы подразделов** | `type` соответствует содержимому: `text` для текста, `table` для моделирования/понятий, `checklist` для заданий/вопросов/ДЗ, `multiple_choice` для задач | да |
| 5 | **Английские имена** | Все папки и файлы — на английском (не транслитерация), kebab-case, без точек | да |
| 6 | **Изображения** | Все `![...]()` ссылки указывают на существующие файлы в `assets/`; именование `fig-NN-*` | да |
| 7 | **Сноски** | Каждая ссылка `[^N]` в тексте имеет определение `[^N]:` в конце файла; нет «осиротевших» определений | да |
| 8 | **index.md** | Есть в корне курса и в каждой папке-секции; содержит `title` и `order`, но не `type` | да |
| 9 | **Pandoc-артефакты** | Нет `{.underline}`, `{.mark}`, `{width=...}`; нет `---` или `--` вместо `—`; нет `**` / `*` в YAML title; нет bold-артефактов (`word**.**`, `****.**`, `**\**`) | да |
| 10 | **Имена файлов** | kebab-case, начинаются с `NN-`, без точек в именах папок (`01-name` а не `1.1-name`) | да |
| 11 | **Grid-таблицы** | Нет grid-таблиц Pandoc (`+---+---+`); все таблицы в pipe-формате (`\| --- \| --- \|`) | да |
| 12 | **Simple-таблицы** | Нет simple-таблиц Pandoc (строки из тире с пробелами); все таблицы в pipe-формате | да |
...
3. **Python (validate.py)** — проверяет результат по 12 правилам format-guide.md
...
- Bold-артефакты вокруг точки: `word**.**` → `word.`, `****.**` → `.**`, `**\**` (standalone) → удалить
- Grid-таблицы (`+---+---+`) → pipe-формат (`| --- | --- |`)
- Simple-таблицы (строки из тире с пробелами) → pipe-формат
...
- **scripts/convert_word/validate.py** — валидатор (12 проверок)
- **scripts/convert_word/fix_grid_tables.py** — конвертация grid-таблиц Pandoc в pipe-формат
- **scripts/convert_word/fix_simple_tables.py** — конвертация simple-таблиц Pandoc в pipe-формат
- **scripts/convert_word/fix_bold_artifacts.py** — исправление bold-артефактов (`word**.**`, `**\**`)
