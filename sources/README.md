...
После конвертации рекомендуется пройти по полному чек-листу. Валидатор (`validate.py`) покрывает часть проверок автоматически, остальные — визуально.
...
| 4 | **Типы подразделов** | `type` соответствует содержимому: `text` для текста, `table` для моделирования/понятий, `checklist` для заданий/вопросов/ДЗ, `multiple_choice` для задач | визуально |
...
| 9 | **Pandoc-артефакты** | Нет `{.underline}`, `{.mark}`, `{width=...}`; нет `---` или `--` вместо `—` в заголовках и тексте; нет `**` / `*` в YAML title | да |
