...
[^563]: С искусственным интеллектом пока не совсем так. В то время как обычная инженерная система изначально создается под выполнение определенных «промптов из окружения». То есть система «стул» по определению будет выполнять ранее назначенную ему функцию.
