...
def clean_pandoc_markup(text):
    ...
    - Bold-артефакты вокруг точки: ****.**  **\**  word**.**
    """
    ...
    # --- Bold-артефакты вокруг точки ---

    # ****.**  → .**  (слияние: **word****.**  → **word.**)
    text = re.sub(r"\*\*\*\*\.\*\*", ".**", text)

    # **word**[^N]**.** → **word[^N].**  (сноска разрывает bold)
    text = re.sub(
        r"\*\*([^*]+)\*\*(\[\^\d+\])\*\*\.\*\*", r"**\1\2.**", text
    )

    # [^N]**.** → [^N].  (оставшиеся сноски перед bold-точкой)
    text = re.sub(r"(\[\^\d+\])\*\*\.\*\*", r"\1.", text)

    # Нумерованные списки: N. word**.** → N. **word.**
    text = re.sub(
    ...
    # Оставшиеся X**.** → X.  (X = любой символ кроме *)
