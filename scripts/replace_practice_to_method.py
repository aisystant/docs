...
PHASE2_RULES = [
    ...
    ('три практики', 'три метода'),
    ('четыре практики', 'четыре метода'),
    # ══════════════════════════════════════════════════════════
    # Additional DATIVE "по" (missing from initial set)
    # ══════════════════════════════════════════════════════════
    ('по такой-то практике', 'по такому-то методу'),
    ('по какой-то практике', 'по какому-то методу'),
    ('по своей основной прикладной практике', 'по своему основному прикладному методу'),
    ('по своей основной практике', 'по своему основному методу'),
    ('по своей практике', 'по своему методу'),
    # ══════════════════════════════════════════════════════════
    # DATIVE "благодаря" + практике → методу
    # ══════════════════════════════════════════════════════════
    ('благодаря данной практике', 'благодаря данному методу'),
    ('благодаря практике', 'благодаря методу'),
    # ══════════════════════════════════════════════════════════
    # Multi-adjective NOMINATIVE (missing)
    # ══════════════════════════════════════════════════════════
    ('отдельная прикладная практика', 'отдельный прикладной метод'),
    ('Отдельная прикладная практика', 'Отдельный прикладной метод'),
    ('ежедневная обязательная практика', 'ежедневный обязательный метод'),
    # ══════════════════════════════════════════════════════════
    # Multi-adjective GENITIVE (missing)
    # ══════════════════════════════════════════════════════════
    ('отдельной прикладной практики', 'отдельного прикладного метода'),
    # ══════════════════════════════════════════════════════════
    # Multi-adjective ACCUSATIVE (missing)
    # ══════════════════════════════════════════════════════════
    ('такую-то практику', 'такой-то метод'),
    ('какую-то практику', 'какой-то метод'),
    ('новую досуговую практику', 'новый досуговый метод'),
    ('новую не сложную практику', 'новый несложный метод'),
    ('любую необходимую практику', 'любой необходимый метод'),
    ('какую прикладную практику', 'какой прикладной метод'),
    # ══════════════════════════════════════════════════════════
    # Multi-adjective INSTRUMENTAL (missing)
    # ══════════════════════════════════════════════════════════
    ('такой же ежедневной обязательной практикой', 'таким же ежедневным обязательным методом'),
    ('вашей главной практикой', 'вашим главным методом'),
    ('отдельной прикладной практикой', 'отдельным прикладным методом'),
    ('обеими практиками', 'обоими методами'),
    # ══════════════════════════════════════════════════════════
    # Multi-adjective PREPOSITIONAL (missing)
    # ══════════════════════════════════════════════════════════
    ('отдельной прикладной практике', 'отдельном прикладном методе'),
    ('своей основной прикладной практике', 'своём основном прикладном методе'),
    # ══════════════════════════════════════════════════════════
    # "одна из" patterns (feminine → masculine)
    # ══════════════════════════════════════════════════════════
    ('одна из основных практик', 'один из основных методов'),
    ('Одна из основных практик', 'Один из основных методов'),
    ('одна из практик', 'один из методов'),
    ('одна из таких практик', 'один из таких методов'),
    ('Одна из таких практик', 'Один из таких методов'),
]
...
PHASE3_RULES = [
    ...
    # ══════ Predicate + "практика" (gender fix on source) ══════
    (r'\bпрактика связана\b', 'метод связан'),
    (r'\bПрактика связана\b', 'Метод связан'),
    (r'\bпрактика необходима\b', 'метод необходим'),
    (r'\bПрактика необходима\b', 'Метод необходим'),
    (r'\bпрактика полезна\b', 'метод полезен'),
    (r'\bпрактика нужна\b', 'метод нужен'),
    (r'\bпрактика важна\b', 'метод важен'),
    (r'\bпоставлена практика\b', 'поставлен метод'),
    (r'\bПоставлена практика\b', 'Поставлен метод'),
    (r'\bзадействована практика\b', 'задействован метод'),
    (r'\bне особо сложная практика\b', 'не особо сложный метод'),
    (r'\bне является особо сложной практикой\b', 'не является особо сложным методом'),
    (r'\bреализовываться сама собой\b', 'реализовываться сам собой'),
    (r'\bреализовываться сама\b', 'реализовываться сам'),
    ...
    # NOTE: numeral patterns "две/три/четыре практики" → Phase 2 literal rules
    # (after 2-4, Russian uses genitive singular "метода", not nom.pl. "методы")
...
PHASE5_POSTFIX = [
    ...
    # ── "благодаря" + dative ──
    (r'\bблагодаря методе\b', 'благодаря методу'),
    (r'\bблагодаря (\w+)ом методе\b', r'благодаря \1ому методу'),
    # Other dative triggers (verbs that take dative):
    (r'\bучиться методе\b', 'учиться методу'),
    (r'\bуделяем методе\b', 'уделяем методу'),
    (r'\bуделить внимание методе\b', 'уделить внимание методу'),
    (r'\bобучаете других методе\b', 'обучаете других методу'),
    (r'\bучить других людей (\w+)ом методе\b', r'учить других людей \1ому методу'),

    # ══════════════════════════════════════════════════════════
    # GENDER FIX: "которая/которой" → "который/которого" (after "метод")
    # ══════════════════════════════════════════════════════════
    ...
    # "целью которой" (after метод)
    (r'\bметод, целью которой\b', 'метод, целью которого'),
    (r'\bметода, целью которой\b', 'метода, целью которого'),
    # With intervening word(s):
    (r'\b(метод[аеуом]?)\s+(\w+)\s+(\w+),\s+которая\b', r'\1 \2 \3, который'),
    (r'\b(метод[аеуом]?)\s+(\w+),\s+которая\b', r'\1 \2, который'),
    (r'\b(метод[аеуом]?)\s+(\w+)\s+которая\b', r'\1 \2 который'),
    (r'\bметодов \w+ \w+, целью которой\b', lambda m: m.group(0).replace('целью которой', 'целью которого')),
    (r'\b(метод[аеуом]?)\s+(\w+)\s+(\w+),\s+целью которой\b', r'\1 \2 \3, целью которого'),
    (r'\b(метод[аеуом]?)\s+(\w+),\s+целью которой\b', r'\1 \2, целью которого'),
    # "о которой идет речь" → "о котором идет речь" (after метод on same line)
    (r'\bметод[аеуом]?\s+\w+,?\s+о которой\b', lambda m: m.group(0).replace('о которой', 'о котором')),

    # ══════════════════════════════════════════════════════════
    # PREDICATE GENDER FIX: fem short adj/participle → masc
    # (when "метод" is the subject on the same line)
    # ══════════════════════════════════════════════════════════
    (r'\bметод связана\b', 'метод связан'),
    (r'\bМетод связана\b', 'Метод связан'),
    (r'\bметод необходима\b', 'метод необходим'),
    (r'\bметод полезна\b', 'метод полезен'),
    (r'\bметод нужна\b', 'метод нужен'),
    (r'\bметод важна\b', 'метод важен'),
    (r'\bметод сложная\b', 'метод сложный'),
    (r'\bметод достаточно сложная\b', 'метод достаточно сложный'),
    (r'\bДанный метод не особо сложная\b', 'Данный метод не особо сложный'),
    (r'\bДанный метод не является особо сложной\b', 'Данный метод не является особо сложным'),
    # "поставлена/задействована метод"
    (r'\bпоставлена метод\b', 'поставлен метод'),
    (r'\bзадействована метод\b', 'задействован метод'),
    (r'\bинтересна метод\b', 'интересен метод'),
    # "метод... будет задействована" → "задействован"
    (r'\bметод\b[^.]*?\bбудет задействована\b', lambda m: m.group(0).replace('будет задействована', 'будет задействован')),
    ...
    (r'\bкоторый призвана\b', 'который призван'),
    (r'\bпоставленная у всех метод\b', 'поставленный у всех метод'),

    # ══════════════════════════════════════════════════════════
    # ADJECTIVE AGREEMENT FIX (leftover feminine endings)
    # ══════════════════════════════════════════════════════════
    # "такая же культурный метод" → "такой же культурный метод"
    (r'\bтакая же культурный метод\b', 'такой же культурный метод'),
    (r'\bтакая же ежедневной обязательным методом\b', 'таким же ежедневным обязательным методом'),
    # "самая важный метод" → "самый важный метод"
    (r'\bсамая важный\b', 'самый важный'),
    # "новую ... метод" → "новый ... метод" (with intervening words)
    (r'\bосваиваете новую досуговый метод\b', 'осваиваете новый досуговый метод'),
    # "никакой более важного" → "никакого более важного"
    (r'\bникакой более важного\b', 'никакого более важного'),
    # "по каждой из 8 изучаемых методов" → "по каждому из 8"
    (r'\bпо каждой из (\d+)\b', r'по каждому из \1'),

    # ══════════════════════════════════════════════════════════
    # "одна из ... методов" → "один из ... методов" (post-replacement safety)
    # ══════════════════════════════════════════════════════════
    (r'\bодна из основных методов\b', 'один из основных методов'),
    (r'\bОдна из основных методов\b', 'Один из основных методов'),
    (r'\bА одна из основных методов\b', 'А один из основных методов'),
    (r'\bодна из методов\b', 'один из методов'),
    (r'\bодна из таких методов\b', 'один из таких методов'),
    (r'\bОдна из таких методов\b', 'Один из таких методов'),
    # "обеими методами" → "обоими методами" (safety net)
    (r'\bобеими методами\b', 'обоими методами'),

    # ══════════════════════════════════════════════════════════
    # PRONOUN FIX: feminine → masculine (specific safe contexts)
    # ══════════════════════════════════════════════════════════
    (r'\bумеете ею\b', 'умеете им'),
    (r'\bвыполнение её принципов\b', 'выполнение его принципов'),
    (r'\bна реализацию и выполнение её принципов\b', 'на реализацию и выполнение его принципов'),
    (r'\bв ее описании\b', 'в его описании'),
    # "понять её основы, чтобы поставить её" → "его ... его"
    (r'\bпонять её основы, чтобы поставить её\b', 'понять его основы, чтобы поставить его'),
    (r'\bпонять его основы, чтобы поставить её\b', 'понять его основы, чтобы поставить его'),
    # "правильно её реализовать" → "его реализовать"
    (r'\bправильно её реализовать\b', 'правильно его реализовать'),
    # "воспринять ее/её всерьез ... инвестировать в нее/неё"
    (r'\bвоспринять ее всерьез, начать инвестировать в нее\b', 'воспринять его всерьез, начать инвестировать в него'),
    (r'\bвоспринять её всерьез\b', 'воспринять его всерьез'),
    (r'\bвоспринять ее всерьез\b', 'воспринять его всерьез'),
    # "отнестись к ней серьезно" → "к нему серьезно"
    (r'\bотнестись к ней серьезно\b', 'отнестись к нему серьезно'),
    # "стать в ней мастером, связав её" → "в нём мастером, связав его"
    (r'\bстать в ней мастером, связав её\b', 'стать в нём мастером, связав его'),
    # "поскольку она повышает" → "он повышает" (follows "данный метод")
    (r'\bпоскольку она повышает\b', 'поскольку он повышает'),
    # "касается она" → "касается он" (after "культурный метод")
    (r'\bкасается она\b', 'касается он'),
    # "её нужно применять" → "его нужно применять"
    (r'\bеё нужно применять\b', 'его нужно применять'),
    # "она может длиться" → "он может длиться" (after "обязательным методом, хотя")
    (r'\bметодом, хотя она\b', 'методом, хотя он'),
    # "пробовали её" → "пробовали его" (after "методе Помодоро")
    (r'\bПомодоро и даже пробовали её\b', 'Помодоро и даже пробовали его'),
    # "объяснить себе ее важность" → "его важность"
    (r'\bобъяснить себе ее важность\b', 'объяснить себе его важность'),
    # "об её эффективности" → "об его эффективности"
    (r'\bоб её эффективности\b', 'об его эффективности'),
    # "без её участия" → "без его участия"
    (r'\bбез её участия\b', 'без его участия'),
    # "без неё" → "без него" (after "быстрее, чем")
    (r'\bбыстрее, чем без неё\b', 'быстрее, чем без него'),
    # "с ней ... без неё" → "с ним ... без него"
    (r'\bс ней это произойдет быстрее, чем без неё\b', 'с ним это произойдет быстрее, чем без него'),
    # "на нее/неё внимание" → "на него внимание" (after "метод")
    (r'\bна нее внимание\b', 'на него внимание'),
    (r'\bна неё внимание\b', 'на него внимание'),
    # "вкладывали в неё" → "вкладывали в него"
    (r'\bвкладывали в неё\b', 'вкладывали в него'),
    # "ее/её основные понятия" → "его основные понятия"
    (r'\bсокурсниками ее основные\b', 'сокурсниками его основные'),
    (r'\bсокурсниками её основные\b', 'сокурсниками его основные'),
    # "она может ей предшествовать" → "он может ему предшествовать"
    (r'\bона может ей предшествовать\b', 'он может ему предшествовать'),
    # "Необходимо владеть обеими" → "обоими" (already in adj section but safety)
    # "она ... помогает решить"
    (r'\bпроблемы она помогает\b', 'проблемы он помогает'),
    # "Метод ... ее упростим" → "его"
    (r'\bее упростим\b', 'его упростим'),
    (r'\bеё упростим\b', 'его упростим'),
    # "реализовываться сама собой" (safety net if Phase 3 didn't catch)
    (r'\bреализовываться сама собой\b', 'реализовываться сам собой'),
    (r'\bреализовываться сама\b', 'реализовываться сам'),

    # ══════════════════════════════════════════════════════════
    # TITLE FIX: "Как метод X связана" → "связан"
    # (title pattern that appears in multiple files)
    # ══════════════════════════════════════════════════════════
    (r'Как метод ([^с].*?) связана', r'Как метод \1 связан'),
    (r'Как метод (систематического.*?) связана', r'Как метод \1 связан'),
    (r'Как метод (инвестирования.*?) связана', r'Как метод \1 связан'),
    (r'Как метод (мышления.*?) связана', r'Как метод \1 связан'),
    (r'Как метод (организации.*?) связана', r'Как метод \1 связан'),
    (r'Как метод (формирования.*?) связана', r'Как метод \1 связан'),
    (r'Как метод (стратегирования.*?) связана', r'Как метод \1 связан'),
    (r'Как метод (планирования.*?) связана', r'Как метод \1 связан'),

    # ══════════════════════════════════════════════════════════
    # GENITIVE FIX: "методы" (fem.gen.sg.) → "метода" (masc.gen.sg.)
    # When "методы" is used as genitive singular (should be "метода")
    # ══════════════════════════════════════════════════════════
    (r'\bпринцип методы\b', 'принцип метода'),
    (r'\bвремя методы\b', 'время метода'),
    (r'\bстадиях методы\b', 'стадиях метода'),
    (r'\bрезультат методы\b', 'результат метода'),
    (r'\bпостановке методы\b', 'постановке метода'),
    (r'\bпонимание методы\b', 'понимание метода'),
    (r'\bДля методы\b', 'Для метода'),
    (r'\bплану методы\b', 'плану метода'),
    (r'\bграниц методы\b', 'границ метода'),
    (r'\bобщее время методы\b', 'общее время метода'),
]
...
def process_line(line: str) -> tuple[str, int]:
    ...
    # Check if line has any "практик" or needs Phase 5 fixes
    lower = line.lower()
    if 'практик' not in lower and 'Практик' not in line:
        # Still apply Phase 5 post-fixes to lines with "метод" (for gender/case fixes)
        if 'метод' in lower:
            result, c5 = apply_rules(line, PHASE5_POSTFIX, is_regex=True)
            if result != line:
                return result, c5
        return line, 0
...
def main():
    ...
    if not dry_run:
        ...
        # Report gender agreement issues (comprehensive)
        print(f"\n{'='*60}")
        print("Checking for gender agreement issues...")
        gender_issues = 0
        gender_patterns = [
            r'\b\w+ая метод\b',       # fem nom adj + метод
            r'\b\w+ую метод\b',       # fem acc adj + метод
            r'\bметод связана\b',      # fem predicate
            r'\bметод необходима\b',
            r'\bметод полезна\b',
            r'\bметод сложная\b',
            r'\bметод достаточно сложная\b',
            r'\bпоставлена метод\b',
            r'\bзадействована метод\b',
            r'\bпризвана устранить метод\b',
            r'\bкоторая призвана\b',
            r'\bтакая же \w+ метод\b',
            r'\bсамая важный\b',
            r'\bодна из \w+ методов\b',
            r'\bобеими методами\b',
            r'\bметод, которая\b',
            r'\bметод[аеуом]?\s+\w+,?\s*которая\b',
            r'\bцелью которой\b',
        ]
        for filepath in md_files:
            with open(filepath, 'r', encoding='utf-8') as f:
